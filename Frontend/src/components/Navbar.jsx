    ...
    <div className='flex items-center justify-between py-5 font-medium'>
        ...
        <div className='flex items-center gap-6'>
            ...
            <div className='group relative'>
                ...
                <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4'>
                    <div className='flex flex-col rounded gap-2 w-40 py-3 px-5 bg-slate-100 text-gray-500 '>
                        {user ? (
                          <>
                            <p className='text-black'>Hi, {user.name}</p>
                            <Link to='/orders' className='cursor-pointer hover:text-black'>Orders</Link>
                            {user.role==='admin' && <Link to='/admin' className='cursor-pointer hover:text-black'>Admin</Link>}
            ...
            <Link to="/cart" className='relative'>
                ...
                <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>
                  {cart?.reduce((s,c)=> s + (c.quantity||0), 0) || 0}
                </p>
            </Link>
        ...
        <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible?'w-full':'w-0'}`}>
            <div  className='flex flex-col text-gray-600'> 
                ...
                <NavLink onClick={()=>{setVisible(false)}} className="py-2 pl-4 border" to="/collection">Collections</NavLink>
